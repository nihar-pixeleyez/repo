                    ...
                    <template v-for="(item, key) in sidebarData" :key="key">
                        ...
                        <template v-else-if="item.subItems">
                            <li v-if="isSidebarSize" class="menu nav-item">
                                <a href="#" @click="toggleSubMenu(key)" :class="{ 'active': activeMenu === key || item.subItems.some(subitem => $route.path === subitem.link) }" class="items-center justify-between text-black nav-link group">
                            ...
                            <ul v-if="isSidebarSize && activeMenu === key" class="flex flex-col gap-1 text-black sub-menu dark:text-white/60">
                        ...
                        <template v-else>
                            <li class="menu nav-item">
                                <router-link :to="item.link" @click="toggleSubMenu(key)" :class="{ 'active': $route.path === item.link }" class="items-center justify-between text-black nav-link group">
                                    <div class="flex items-center">
                                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" class="w-5 h-5">
                                            <path :d="item.icon" fill="currentColor"></path>
                                        </svg>
